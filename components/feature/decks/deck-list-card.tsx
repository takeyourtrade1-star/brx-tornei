...
const STATUS_META: Record<Status, { label: string; className: string; Icon: typeof ShieldCheck }> = {
  verified: { label: 'Verificato', className: 'border-emerald-400/30 bg-emerald-500/20 text-emerald-200', Icon: ShieldCheck },
  mismatch: { label: 'Da controllare', className: 'border-red-400/30 bg-red-500/20 text-red-200', Icon: AlertTriangle },
  legal: { label: 'Pronto', className: 'border-emerald-400/30 bg-emerald-500/20 text-emerald-200', Icon: ShieldCheck },
  building: { label: 'In costruzione', className: 'border-amber-400/30 bg-amber-500/20 text-amber-200', Icon: Hammer },
};
...
export function DeckListCard({ deck, onEdit, onDelete }: {
  ...
  return (
    <article className="group relative overflow-hidden rounded-3xl border border-white/10 bg-slate-950/90 shadow-xl shadow-black/40 transition duration-300 hover:-translate-y-1 hover:border-white/20 hover:shadow-2xl hover:shadow-black/50">
      <div className="relative h-32 overflow-hidden border-b border-white/10">
        <span className="absolute inset-0 bg-cover bg-center transition duration-500 group-hover:scale-105" style={{ backgroundImage: `url(${FORMAT_MEDIA[deck.formatId].image})` }} aria-hidden />
        <span className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent" aria-hidden />

        <div className="relative flex h-full flex-col justify-between p-4">
          <div className="flex items-start justify-between gap-3">
            <span className="rounded-full border border-white/20 bg-black/40 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-white backdrop-blur-md">
              {format?.name ?? deck.formatId}
            </span>
            <div className="flex items-center gap-1.5">
              <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-wide backdrop-blur-md ${className}`}>
                <Icon className="h-3 w-3" aria-hidden />
                {label}
              </span>
              <button type="button" onClick={() => setConfirmDelete(true)} aria-label={`Elimina ${deck.name}`} className="grid h-7 w-7 place-items-center rounded-full border border-white/15 bg-black/40 text-white/55 backdrop-blur-md transition hover:border-red-400/40 hover:bg-red-500/25 hover:text-red-200">
          ...
          <div>
            ...
            <p className="mt-0.5 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-white/55">
      ...
      <div className="p-4">
        ...
        <button type="button" onClick={onEdit} className="mt-4 flex w-full items-center justify-between rounded-xl border border-white/10 bg-white/[0.055] px-3.5 py-2.5 text-xs font-black uppercase tracking-wide text-white transition hover:border-white/20 hover:bg-white/10">
...
function DeckStat({ value, label, accent = false }: { value: string; label: string; accent?: boolean }) {
  return (
    <div className="rounded-xl border border-white/[0.07] bg-white/[0.035] px-2 py-2 text-center">
