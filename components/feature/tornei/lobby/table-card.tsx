...
export function TableCard({ table, busy, onSit, onOpen, onLeave, onGoLive }: TableCardProps) {
  ...
  return (
    <article
      className={cn(
        'relative isolate overflow-hidden rounded-[2rem] border bg-gradient-to-br from-footer-start via-card2-end to-card2-end p-4 text-white shadow-xl shadow-card2-end/25 transition duration-200 sm:p-5',
        isMine
          ? 'border-primary/45 ring-1 ring-primary/15'
          : 'border-white/10 hover:-translate-y-0.5 hover:border-white/25 hover:from-card4-start',
      )}
    >
      <span
      ...
      />
      {isMine && (
        <span className="absolute inset-y-7 left-0 w-1 rounded-r-full bg-primary" aria-hidden="true" />
      )}
