...
/** 8 caratteri alfanumerici minuscoli — copia fedele da new_frontend_brx. */
export function EmailCodeInput({
  ...
  const slotClass = cn(
    'h-10 w-10 rounded-2xl border border-black/10 bg-black/5 text-center text-base font-semibold text-[#1d1d1f] shadow-[inset_0_1px_2px_rgba(0,0,0,0.03)] transition-all duration-150 focus:border-[#0066cc] focus:bg-white/90 focus:outline-none focus:ring-2 focus:ring-[#0066cc]/20 disabled:cursor-not-allowed disabled:opacity-40'
  );

  return (
    <div id={id} className={cn('w-full px-4', className)}>
      <div className="flex items-center justify-center gap-2">
        {Array.from({ length: 8 }).map((_, i) => (
          <input
            ...
            onPaste={handlePaste}
            className={slotClass}
            aria-label={`Carattere codice ${i + 1}`}
          />
        ))}
      </div>
    </div>
