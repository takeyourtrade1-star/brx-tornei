...
const REQUIRED_PRODUCTION_VALUES = [
  ...
  'UPSTASH_REDIS_ALLOWED_HOSTNAME',
...
export function validateProductionEnvironment(environment) {
  ...
  for (const name of [...REQUIRED_PRODUCTION_ORIGINS, ...OPTIONAL_HTTPS_ORIGINS]) {
  ...
  for (const name of ['AUTH_API_URL', 'SYNC_API_URL', 'TOURNAMENTS_API_URL']) {
    if (!trustedUpstreamHostnames.has(new URL(origins[name]).hostname)) {
      throw new Error(`${name} hostname is not in TRUSTED_UPSTREAM_HOSTS`);
    }
  }
