...
/**
 * Descrizione redatta della configurazione ricevuta, stampata con il rifiuto.
 *
 * A validator that only says "no" turns one wrong character into a round trip
 * per attempt, and a build takes minutes. I valori raw non vengono mai inclusi:
 * un origin errato può contenere userinfo, query o frammenti sensibili.
 *
...
function describeOriginConfiguration(
  ...
  for (const name of [
    ...
    try {
      ...
      lines.push(`    ${name} = (configured; not a URL)`);
      continue;
    }
    const verdict = trustedHostnames.has(hostname) ? 'trusted' : 'NOT TRUSTED';
    lines.push(`    ${name} = (configured; host ${hostname}: ${verdict})`);
  }
