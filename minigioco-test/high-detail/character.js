...
import { isAutomaticBlink } from "./character-motion.js";
 ...
 * @param {{ x: number, y: number, look?: { hair?: string, outfit?: string }, direction?: string, time?: number, walking?: boolean, seated?: boolean, blink?: boolean, ghost?: boolean, reducedMotion?: boolean, motionTime?: number }} options
 */
export function drawDetailedCharacter(ctx, options = {}) {
  ...
  const safeMotionTime = Number.isFinite(motionTime) ? motionTime : safeTime;
  const style = resolveCharacterStyle(look);
  const facing = resolveDirection(direction);
  const metrics = poseMetrics(safeTime, walking, seated, reducedMotion, safeMotionTime);
  const anchor = { x, y };
  const ground = { x, y };
  const shouldBlink = !reducedMotion && (Boolean(blink) || isAutomaticBlink(safeTime, reducedMotion));
  ...
  drawDetailedHead(ctx, anchor, style, facing, metrics, shouldBlink, ghost);
  ctx.restore();
}
